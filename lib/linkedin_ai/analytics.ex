defmodule LinkedinAi.Analytics do
  ...
  def get_user_content_analytics(%User{} = user) do
    ...
    published_count =
      from(gc in base_query, where: gc.is_published == true, select: count(gc.id)) |> Repo.one()

    favorites_count =
      from(gc in base_query, where: gc.is_favorite == true, select: count(gc.id)) |> Repo.one()

    # Content by type
    content_by_type =
      from(gc in base_query,
  ...
  def get_user_profile_analytics(%User{} = user) do
    ...
    implemented_count =
      from(pa in base_query, where: pa.status == "implemented", select: count(pa.id))
      |> Repo.one()

    # Latest score
    latest_score =
      from(pa in base_query,
  ...
  def get_user_engagement_analytics(%User{} = user) do
    ...
    avg_engagement =
      if length(published_content) > 0 do
    ...
    top_content =
      from(gc in GeneratedContent,
        ...
        order_by: [
          desc:
            fragment(
              "(engagement_metrics->>'likes')::int + (engagement_metrics->>'comments')::int + (engagement_metrics->>'shares')::int"
            )
        ],
        limit: 5,
        select: %{
          ...
          engagement_metrics: gc.engagement_metrics,
  ...
  def get_user_platform_analytics do
    ...
    active_users =
      from(u in User, where: u.account_status == "active", select: count(u.id)) |> Repo.one()
  ...
  def get_subscription_platform_analytics do
    ...
    active_subscriptions =
      from(s in Subscription, where: s.status == "active", select: count(s.id)) |> Repo.one()
  ...
  def get_content_platform_analytics do
    ...
    published_content =
      from(gc in GeneratedContent, where: gc.is_published == true, select: count(gc.id))
      |> Repo.one()
  ...
  def get_revenue_analytics do
    ...
    %{
      ...
      avg_revenue_per_user:
        if(basic_count + pro_count > 0,
          do: Decimal.div(total_mrr, basic_count + pro_count),
          else: Decimal.new("0")
        )
    }
  end
  ...
  def get_platform_usage_analytics do
    ...
    %{
      ...
      avg_usage_per_user:
        if(active_users_count > 0, do: Float.round(total_usage / active_users_count, 1), else: 0)
    }
  end
  ...
  defp get_monthly_usage_for_period(user_id, period_start, period_end) do
    from(ur in UsageRecord,
      where:
        ur.user_id == ^user_id and ur.period_start >= ^period_start and
          ur.period_end <= ^period_end,
      group_by: ur.feature_type,
      select: {ur.feature_type, sum(ur.usage_count)}
    )
    |> Repo.all()
  ...
  defp generate_achievements(analytics) do
    ...
    achievements =
      if analytics.content.total_generated >= 50 do
    ...
    achievements =
      if analytics.profile.avg_score >= 80 do
    ...
    achievements =
      if analytics.content.publish_rate >= 50 do
